...
def _assure(args: argparse.Namespace) -> int:
    ...
    if result.assurance_status == "VERIFIED":
        return 0
    return 1 if result.fallback_reason in {"import_error", "export_error", "isolated_worker_error"} else 2
    ...
    try:
        ...
        if args.optimize:
            if args.output is None:
                print("rqm-qiskit: --output is required with --optimize", file=sys.stderr)
                return 1
...
def _parser() -> argparse.ArgumentParser:
    ...
    analyze = subparsers.add_parser("analyze", help="Create a geometric circuit report.")
